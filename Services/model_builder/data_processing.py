def preprocess_data(dados, scenario):
    ...
    dados.RF.loc[(dados.RF['C'].isin(dados.feeder)) & (dados.RF['K'].isin(dados.K_40pes))] = dados.slot_cost * (1 + 0.25) * 2
    dados.RF.loc[(dados.RF['C'].isin(dados.feeder)) & ~(dados.RF['K'].isin(dados.K_40pes))] = dados.slot_cost * (1 + 0.25) * 1
